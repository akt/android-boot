...
public class LineChartRenderer extends LineRadarRenderer {
     ...
     */
    protected Paint mCirclePaintHole;

    protected Paint mCirclePaintInner;

    protected Paint mAverageLinePaint;

    protected Paint mAverageLabelPaint;
    ...
    public LineChartRenderer(LineDataProvider chart, ChartAnimator animator,
        ...
        mCirclePaintHole = new Paint(Paint.ANTI_ALIAS_FLAG);
        mCirclePaintHole.setStyle(Paint.Style.FILL);
        mCirclePaintHole.setColor(Color.WHITE);
        ...
        mAverageLinePaint.setColor(Color.BLACK);
        mAverageLinePaint.setStrokeWidth(2f);
        mAverageLinePaint.setStyle(Paint.Style.STROKE);

        mAverageLabelPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mAverageLabelPaint.setColor(Color.BLACK);
        mAverageLabelPaint.setStyle(Paint.Style.FILL);
        mAverageLabelPaint.setTextSize(Utils.convertDpToPixel(13f));
        mAverageLabelPaint.setTextAlign(Paint.Align.LEFT);
    ...
    private void drawAverageLine(Canvas c) {
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ILineDataSet lineDataSet = dataSets.get(i);
            float averageY = lineDataSet.getYAverage();
            float formatAverage = (float) (Math.round(averageY * 100)) / 100;
            float phaseY = mAnimator.getPhaseY();
            float y = formatAverage * phaseY;
            ...
            mAverageLinePaint.setColor(lineDataSet.getAverageLineColor());
            c.drawLine(buffer[0],
                    buffer[1], mViewPortHandler.contentRight(),
                    buffer[1], mAverageLinePaint);

            mAverageLabelPaint.setColor(lineDataSet.getAverageLineLabelColor());
            String label = "平均值" + formatAverage;
            float textStartX = buffer[0] + Utils.convertDpToPixel(12f);

            float labelLineHeight = Utils.getLineHeight(mAverageLabelPaint, legendFontMetrics);
            c.drawText(label, textStartX, buffer[1] - labelLineHeight / 2, mAverageLabelPaint);
        }
    ...
    protected void drawCircles(Canvas c) {
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            mCirclePaintHole.setColor(dataSet.getCircleHoleColor());
            ...
            float circleInnerRadius = dataSet.getCircleInnerRadius();
...
//            drawHighlightLines(c, (float) pix.x, (float) pix.y, set);
            ...
            mHighlightCirclePaint.setColor(set.getColor());
            c.drawCircle((float) pix.x, (float) pix.y, Utils.convertDpToPixel(3f), mHighlightCirclePaint);
        }
    }
    ...
    private class DataSetImageCache {
        ...
        protected void fill(ILineDataSet set, boolean drawCircleHole, boolean drawTransparentCircleHole) {
            ...
            float circleInnerRadius = set.getCircleInnerRadius();
