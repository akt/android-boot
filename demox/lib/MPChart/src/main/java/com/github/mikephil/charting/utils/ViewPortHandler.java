...
public class ViewPortHandler {
     ...
     *
     * @param direction
     * @return direction Negative to check scrolling left, positive to check scrolling right. //copy的
     */
    public boolean isOverBoundOnX(int direction) {
        mMatrixTouch.getValues(matrixBuffer);
        ...
        if (mContentRect != null) {
            width = mContentRect.width();
        }

        float maxTransX = -width * (mScaleX - 1f);

        if (direction < 0) {
            return curTransX >= mTransOffsetX;
        } else if (direction > 0) {
            return curTransX <= maxTransX - mTransOffsetX;
        } else {
            return false;
        }
    }
