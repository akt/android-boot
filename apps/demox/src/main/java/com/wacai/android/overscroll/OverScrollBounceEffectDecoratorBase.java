package com.wacai.android.overscroll;
...
import static com.wacai.android.overscroll.IOverScrollState.STATE_BOUNCE_BACK;
import static com.wacai.android.overscroll.IOverScrollState.STATE_DRAG_END_SIDE;
import static com.wacai.android.overscroll.IOverScrollState.STATE_DRAG_START_SIDE;
import static com.wacai.android.overscroll.IOverScrollState.STATE_IDLE;
import static com.wacai.android.overscroll.ListenerStubs.OverScrollStateListenerStub;
import static com.wacai.android.overscroll.ListenerStubs.OverScrollUpdateListenerStub;
...
public abstract class OverScrollBounceEffectDecoratorBase implements IOverScrollDecor, View.OnTouchListener {
    ...
    public static final float DEFAULT_TOUCH_DRAG_MOVE_RATIO_FWD = 2.5f;
    ...
    protected class OverScrollingState implements IDecoratorState {
        ...
        public OverScrollingState(Context context, float touchDragRatioFwd, float touchDragRatioBck) {
            mContext = context;
            mMoveAttr = createMotionAttributes();
    ...
    public OverScrollBounceEffectDecoratorBase(IOverScrollDecoratorAdapter viewAdapter, float decelerateFactor, float touchDragRatioFwd, float touchDragRatioBck) {
        ...
        Context context;
        if (mViewAdapter.getView() != null) {
            context = mViewAdapter.getView().getContext();
        } else {
            context = mViewAdapter.getInsideView().getContext();
        }
        mOverScrollingState = new OverScrollingState(context, touchDragRatioFwd, touchDragRatioBck);
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                boolean resultMove = mCurrentState.handleMoveTouchEvent(event);
                if (resultMove) {
                    setLongClickable(getView(), false);
                    setLongClickable(getInsideView(), false);
                }
                return resultMove;
            ...
            case MotionEvent.ACTION_UP:
                upX = (int) event.getX();
                upY = (int) event.getY();
                setLongClickable(getView(), true);
                setLongClickable(getInsideView(), true);
                mCurrentState.handleUpOrCancelTouchEvent(event);
                boolean resultUp = upX - downX > mTouchSlop;
                if (resultUp) {
    ...
    public View getInsideView() {
        return mViewAdapter.getInsideView();
    }
    ...
    protected void attach() {
        ...
        if (getInsideView() != null) {
            getInsideView().setOnTouchListener(this);
            getInsideView().setOverScrollMode(View.OVER_SCROLL_NEVER);
        }
    }
    ...
    public void detach() {
        ...
        if (mViewAdapter.getInsideView() != null) {
            mViewAdapter.getInsideView().setOnTouchListener(null);
            mViewAdapter.getInsideView().setOverScrollMode(View.OVER_SCROLL_ALWAYS);
        }
    }

    private void setLongClickable(View view, boolean longClickable) {
        if (view != null) {
